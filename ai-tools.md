...
graph TD
    ...
    LLM[LLM]:::highlight --> Tokenization[Tokenization]
    
    subgraph Text Processing
        Tokenization --> WordToNum["word -> numb"]
        WordToNum --> Chunks["chunks (chunk1, chunk2)"]
        Chunks --> Tokens["tokens (token1, token2)"]
    end

    subgraph ML Representation
        ML[ML]:::highlight -->|creates| Embeddings[embeddings]
        ML -->|goal| Goal["Next words prediction"]:::goal
        Chunks --> Embeddings
        Embeddings --> Relation["relation (vectors)"]:::highlight
    end

    subgraph Output Generation
        Goal --> NN((Neural Network))
        Relation --> NN
        NN --> Goal
    end
```
...
*   **ML (Machine Learning):** The underlying technology.
*   **Goal:** Next words prediction (predicting the next token based on previous context).
*   **Mechanism:** Uses a neural network to process inputs and generate outputs.

**2. Data Preprocessing Pipeline**
*   **Tokenization:** Breaking down raw text into smaller pieces.
*   **Word to Number (`word -> numb`):** Converting human-readable text into numerical representations the model can understand.
*   **Chunks & Tokens:** Grouping text into chunks (`chunk1`, `chunk2`), which are further divided into individual tokens (`token1`, `token2`).

**3. Representation & Relationships**
*   **Embeddings:** Mathematical vectors that represent tokens in a high-dimensional space. 
*   **Creation:** Generated by the ML process.
*   **Relations:** The spatial distance and direction between embeddings (vectors) capture the semantic relationships between words/concepts.

**4. Summary Flow**
`Raw Text` ➔ `Tokenization` ➔ `Word to Number` ➔ `Chunks/Tokens` ➔ `Embeddings` ➔ `Capture Relations` ➔ `Neural Network` ➔ `Next Word Prediction`
